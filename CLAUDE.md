...
**Risolti (2026-08-30) — curation del registry, wizard dispositivi e meteo da HA:**
- **Una sola curation per ogni superficie utente** — `useHAHiddenEntities` (solo `hidden_by`) è sostituito da [useDashboardEntityCuration](src/hooks/useDashboardEntityCuration.ts), che passa dal kernel puro [entityCuration.ts](src/lib/entityCuration.ts): il registry HA è l'unica autorità su `hidden_by`, `disabled_by` ed `entity_category` (`config`/`diagnostic`), più le impostazioni-camera travestite da `switch.*` (ezviz/ring) e i **relè duplicati** — uno `switch` omonimo di una `light` non appare più accanto alla card semantica. Il filtro `entity_category` sugli attributi di stato (che non li contiene mai: vive nel registry) è rimosso da composer, stanze e discovery. L'override `enabled: true` ora **riammette** esplicitamente un'entità esclusa dal registry.
- **Wizard "Configura dispositivo"** — [DeviceSetupWizard](src/components/entities/DeviceSetupWizard.tsx) in Entità: 4 passi (dispositivo → categoria → stanza → verifica con anteprima live della card XL). La tassonomia completa delle 25 card vive in [deviceSetup.ts](src/lib/deviceSetup.ts) ed è condivisa con l'editor di riga (prima ne elencava solo 17, le altre 8 erano irraggiungibili dalla UI). `applyDeviceSetup` tocca solo categoria e stanza, conservando ogni altra cura della card.
- **Stanza assegnabile a mano** — `DeviceOverride.areaId` (validato in `config-validation.ts`, max 128 char). Precedenza in [areaInference.ts](src/lib/areaInference.ts): scelta manuale valida → registry HA → **inferenza prudente** per nome (token dell'area nel friendly name, dominio deliberatamente ignorato perché `camera.` non implica la camera da letto; in caso di pareggio non indovina). Le righe di Entità marcano in blu la stanza assegnata a mano.
- **Meteo senza OpenWeather** — [haWeather.ts](src/lib/haWeather.ts) adatta l'entità `weather.*` nativa di HA al contratto kiosk esistente (condizione → icona, vento normalizzato da m/s e mph). `useWeather` interroga OpenWeather solo se la chiave è configurata e degrada al meteo di HA: una casa senza `OPENWEATHER_API_KEY` non resta più senza meteo.
- **Rumore che non è più un allarme** — `isRelevantUnavailableEntity`: `unavailable` diventa notifica solo per un elettrodomestico comandabile o un sensore di sicurezza/accesso, mai per telemetria di sfondo e tracker. La chip del composer scatta da 3 (non 5), è `warn` e **porta con sé gli entityId** invece di essere un vicolo cieco. Le batterie restano proattive anche quando HA classifica il loro sensore come diagnostico (`device_class: battery` letto anche dallo stato).
- **Card**: `binary_sensor` `device_class: problem` non è più spacciato per perdita d'acqua (famiglia `system`, "Problema rilevato"); gruppi in `XL` su due colonne con stati in italiano (`groupMemberStateLabel`, non più `on`/`off` grezzi) e leggibili in dark; rifiuti mostra i ritiri futuri anche in `XL`. Il composer satura il canvas 3×6 con pattern per 1–4 contenuti, così nessuna card finisce oltre il clipping.
- **Fix all'identità dell'indice aree** — `useAreaIndex` legge lo store al momento della chiamata invece di sottoscriverlo: `areaIdOf`/`areaNameOf` restano stabili fra i delta HA, altrimenti `useComposedHome` ricreava subscription e tick d'isteresi a ogni aggiornamento di stato del tablet. L'inferenza è cache-ata per nome.
- Verifica: lint ✅ · test 372/372 ✅ · build:all ✅ · typecheck backend ✅. **Da provare sul tablet reale:** il wizard con entità HA vive, il meteo da entità `weather.*` e la curation su un'installazione con molte entità diagnostiche.


**Risolti (2026-08-30, sera) — regia operativa: via il rumore, dentro ciò che serve:**
- **"Chi è offline" diventa una diagnosi** — Stato contava *tutte* le entità `unavailable` (27 su un'installazione reale) e Sistema ne elencava 36 con l'entity_id grezzo come nome: in cima c'erano sette tracker BLE fantasma fermi da 5 giorni, e l'unico guasto vero (`climate.cucina_soggiorno`, giù da 40 minuti) era sepolto. Ora il kernel puro [offlineDevices.ts](src/lib/offlineDevices.ts) applica lo stesso criterio delle notifiche (`isRelevantUnavailableEntity`) e **raggruppa per integrazione** — quando un'integrazione cade cadono tutte le sue entità insieme, quindi "sonoff · 3 dispositivi" è la diagnosi mentre tre righe separate sono rumore. La telemetria di sfondo resta un numero onesto in fondo, mai in evidenza. Card condivisa [OfflineDevicesCard](src/components/system/OfflineDevicesCard.tsx), gruppi aperti di default (una risposta che richiede un click non è una risposta).
- **Salute nel tempo, non solo "risponde adesso"** — nuovo [service-health.ts](backend/src/lib/service-health.ts): uptime del servizio, da quanto il ponte dati è stabile, quante volte è caduto e perché. Le cadute sono **idempotenti** (WS e poll segnalano la stessa caduta: si conta una volta). Esposto in `GET /api/system/status` come `health`.
- **Errori recenti** — il log registrava solo le azioni critiche *riuscite*; cosa falliva non lasciava traccia. Ora un ring buffer da 60 voci raccoglie comandi HA rifiutati (con entità e codice), cadute del ponte e HA irraggiungibile, esposto da `GET /api/system/errors`. Errori consecutivi identici **collassano in una voce con contatore**: un HA giù per un'ora resta una riga, non riempie il buffer nascondendo tutto il resto.
- **Aggiornamenti disponibili** — [update-check.ts](backend/src/lib/update-check.ts) legge la versione pubblicata dal manifest dell'add-on su `main` (URL fisso, nessun input utente, cache 6h, una sola richiesta in volo). Il confronto lo fa il client, l'unico a conoscere la propria `__APP_VERSION__`; in build `dev` non si annuncia nulla.
- **Le due viste smettono di duplicarsi** — Stato = *cosa non va in casa* (connessione, problemi di config, dispositivi offline, backup); Sistema = *come sta il servizio* (connessione HA, [ServiceHealthCard](src/components/system/ServiceHealthCard.tsx), tablet, azioni critiche). Rimossi da Stato i titoloni su numeri d'impianto (modo del ponte, client SSE, `lastEventId`: un contatore monotono che non cambia una decisione); rimossi da Sistema il dump di 30 entità diagnostiche, la lista grezza delle non disponibili e la card "Storage & versione" che ripeteva Stato. `SystemPage` 417 → 295 righe.
- **Bug trovato provando davvero** — guidando Chrome via CDP la regia è caduta sull'error boundary: `status?.health.connectedSince` proteggeva `status` ma non `health`, assente quando il backend è più vecchio del bundle — cioè **durante ogni aggiornamento dell'add-on**. `health` è ora opzionale nel tipo e la vista degrada a "—".
...
**Risolti (2026-09-04) — cronologia layout, "Attenzione" unificata, scene con riscontro, insight consumi:**
- **Cronologia layout con rollback non distruttivo (chiude §4.4)** — ogni salvataggio di `config.home` (tablet via `/api/layout/home`, regia via `PUT /api/config`) registra ora una revisione in `store.homeRevisions` ([backend/src/lib/home-revisions.ts](backend/src/lib/home-revisions.ts): diff puro widget-per-widget — aggiunti/rimossi/spostati/ridimensionati/riordinati — capped a 10, un salvataggio "edit" senza differenze osservabili non produce voce). `GET/POST /api/system/home-revisions` (desktop-only); il **ripristino non sovrascrive**: crea una nuova revisione con `source:'rollback'` + `restoredFromVersion`, riusando `mergeHomeConfig` (stessa disciplina `layoutVersion` di sempre, niente percorso di scrittura parallelo). Card "Versioni della home" in Sistema. Provato end-to-end contro un backend reale con DB usa-e-getta: due modifiche → due revisioni, ripristino alla prima → v4 nuova con `restoredFromVersion:2`, v2/v3 intatte.
- **"Cosa non va" smette di essere il quarto rilevatore indipendente** — c'erano già 5 detector scoordinati (allarmi critici per l'overlay kiosk, dispositivi offline, batteria dentro le notifiche, il chip P0 del composer, e i "problemi" ad hoc di Stato), nessuno con una tassonomia condivisa. [src/lib/attention.ts](src/lib/attention.ts) compone i quattro rilevanti a Stato (allarmi critici, offline, batteria, configurazione) senza duplicare nessuna regola di rilevamento — solo categoria/severità/ordinamento — dietro [AttentionCard](src/components/system/AttentionCard.tsx). Non sopprimibile **per categoria**, non per severità: sicurezza/intrusione restano sempre visibili anche "critiche", una batteria critica invece si può posticipare da chi la conosce già. Posticipo (1h/stasera/domani/finché non cambia) in `localStorage['myhome.attentionSnoozes']` — per-dispositivo come `myhome.home`/`myhome.haStream`, deliberatamente non sincronizzato: è comodità diagnostica, non stato di sicurezza condiviso.
- **Scenari: il tap non spariva più nel nulla** — [SceneRow](src/components/layout/SceneRow.tsx) chiamava `scene.turn_on` senza alcun riscontro. Ora fase `pending → done/failed` per orb (anello che gira, spunta verde, shake sull'errore), niente doppio tap durante l'esecuzione.
- **Insight consumi a regole (M5, coerente: niente ML, niente somme arbitrarie)** — [consumptionInsights.ts](src/lib/consumptionInsights.ts): flusso d'acqua sostenuto ≥30min sopra soglia (solo su sensori a **portata**, `L/min`/`m³/h` — un contatore cumulativo viene ignorato di proposito, un falso "forse perdita" è peggio del silenzio) e autosufficienza solare (un sensore di produzione contro il sensore di consumo già scelto da `EnergyCard`, mai una somma). Capability-gated sull'esistenza di `EnergyCard` stessa (nessun sensore di potenza → niente insight, limite noto).
