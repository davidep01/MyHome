...
export function StatusPage() {
  ...
  const problems = useMemo(() => {
    ...
    return list
  }, [status, unavailable.length, setActiveView])
  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto pr-1">
      <GlassCard className="shrink-0">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className={cn('flex h-11 w-11 items-center justify-center rounded-[14px]', online ? 'bg-green-500/12 text-green-700' : 'bg-red-500/12 text-red-600')}>
              {online ? <CheckCircle2 size={21} /> : <WifiOff size={21} />}
            </div>
            <div>
              <p className="text-lg font-semibold text-[#1d1d1f]">{online ? 'La casa è connessa' : 'Connessione assente'}</p>
              <p className="text-sm text-black/45">
                {status?.ha.reachable
                  ? `Home Assistant risponde in ${status.ha.latencyMs}ms · bridge ${status.stream.mode === 'ws' ? 'WebSocket (push)' : status.stream.mode === 'poll' ? `poll ${status.stream.pollMs}ms` : 'inattivo'}`
                  : status?.ha.message ?? 'Verifica in corso…'}
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <RefreshCw size={15} className={cn(isFetching && 'animate-spin')} /> Prova connessione
            </button>
            <a
              href="/kiosk"
              target="_blank"
      ...
      <div className="grid shrink-0 grid-cols-2 gap-3 xl:grid-cols-4">
        <Metric label="Latenza HA" value={status?.ha.latencyMs != null ? `${status.ha.latencyMs}ms` : '—'} tone={status?.ha.reachable ? 'ok' : 'warn'} Icon={Activity} />
        ...
        <Metric label="Client live" value={String(status?.stream.subscribers ?? 0)} Icon={ArrowUpRight} />
        <Metric label="Storage" value={status ? `${status.storage.mode}${status.storage.writable ? '' : ' (RO)'}` : '—'} tone={status?.storage.writable ? 'neutral' : 'warn'} Icon={Database} />
      </div>

      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 xl:grid-cols-2">
        <GlassCard className="min-h-[260px] space-y-2 overflow-y-auto">
          <p className="text-sm font-semibold text-[#1d1d1f]">Cosa non va</p>
          {problems.length === 0 ? (
            <div className="flex h-[calc(100%-32px)] flex-col items-center justify-center gap-2 text-black/35">
            ...
            problems.map((p) => (
              <div key={p.id} className={cn('flex items-center gap-3 rounded-[12px] px-3 py-2.5', p.severity === 'danger' ? 'bg-red-500/10' : 'bg-orange-500/10')}>
                ...
                {p.action && (
                  <button onClick={p.action} className="shrink-0 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-black/60 active:scale-95">
                    {p.actionLabel}
                  </button>
                )}
              </div>
        ...
        <div className="flex min-h-0 flex-col gap-4">
          <GlassCard className="space-y-2">
            <p className="text-sm font-semibold text-[#1d1d1f]">Attività</p>
            ...
            <InfoRow
              label="Integrazioni"
              value={status ? ['Gemini', 'OpenWeather', 'Supabase'].filter((_, i) => [status.integrations.gemini, status.integrations.openweather, status.integrations.supabase][i]).join(' · ') || 'nessuna chiave' : '—'}
              Icon={KeyRound}
            />
          </GlassCard>
          <BackupCard />
...
function Metric({ label, value, tone = 'neutral', Icon }: { label: string; value: string; tone?: 'neutral' | 'ok' | 'warn'; Icon: React.ElementType }) {
  return (
    <div className={cn(
      ...
      <p className="mt-1 truncate text-2xl font-semibold text-[#1d1d1f]">{value}</p>
    </div>
  )
}
  ...
  return (
    <GlassCard className="space-y-2.5">
      <div>
        ...
        <p className="mt-0.5 text-xs text-black/40">Esporta o ripristina l'intera configurazione (curation, stanze, funzioni) in JSON.</p>
      </div>
      ...
      {message && (
        <p className={cn('text-xs font-medium', message.ok ? 'text-green-700' : 'text-red-600')}>{message.text}</p>
      )}
    </GlassCard>
  )
}
