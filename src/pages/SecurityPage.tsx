...
export function SecurityPage() {
  ...
  return (
    <div className="flex h-full flex-col gap-5 overflow-y-auto pr-1">
      <div>
        <h1 className="text-2xl font-semibold tracking-[-0.01em] text-white sm:text-3xl">Sicurezza</h1>
        <p className="mt-1 text-sm text-white/45">Accessi, allarme e videocamere</p>
      </div>

      <GlassCard glow={status.tone === 'ok' ? undefined : `${status.color}55`} className="min-h-[120px]">
        <div className="flex h-full items-center gap-4">
          <div className="flex h-14 w-14 items-center justify-center rounded-[18px] bg-white/8">
            <Icon size={25} style={{ color: status.color }} />
          </div>
          <div>
            <p className="text-2xl font-semibold tracking-[-0.012em] text-white">{status.label}</p>
            <p className="mt-1 text-sm text-white/40">{status.detail}</p>
          </div>
        </div>
      </GlassCard>
