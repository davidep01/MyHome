    ...
    <div className="flex flex-col gap-2 overflow-y-auto">
      {articles.map((article) => (
        <a
          ...
          className="group flex gap-3 rounded-[14px] bg-white/5 p-3 hover:bg-white/8 transition-colors"
        >
          {article.urlToImage && (
            <img
              src={article.urlToImage}
              alt=""
              className="h-14 w-14 shrink-0 rounded-[10px] object-cover bg-white/10"
              onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }}
            />
          )}
          <div className="flex flex-col gap-1 min-w-0">
            <p className="text-xs font-medium text-white/85 line-clamp-2 leading-snug">
              {article.title}
            </p>
