  ...
  if (!hasAny) {
    return (
      <div className="flex items-center gap-3 text-black/40">
        ...
        <div>
          <p className="text-sm font-semibold text-black/65">Casa tranquilla</p>
    ...
    <div className="flex flex-wrap items-center gap-2.5">
      ...
      {lightsOn > 0 && (
        ...
        >
          ...
          {!compact && <span className="text-[#7a5b08]/70">{lightsOn === 1 ? 'luce accesa' : 'luci accese'}</span>}
          {expanded && <span className="ml-1 flex items-center gap-1 rounded-full bg-black/8 px-2 py-0.5 text-xs text-[#7a5b08]">
      ...
      {avgIndoorTemp !== null && (!compact || lightsOn === 0) && (
        <div className="flex items-center gap-2 rounded-full bg-black/[0.05] py-2.5 px-3.5 text-[15px] font-semibold text-black/70">
          ...
          {!compact && <span className="text-black/40">in casa</span>}
        </div>
      )}
